...
object DatabaseMigrations {
    ...
    /** 修复旧编辑页保存后把Edge预设重置为普通HTTP引擎的问题 */
    @Suppress("ClassName")
    val MIGRATION_72_73 = object : Migration(72, 73) {
        override fun migrate(db: SupportSQLiteDatabase) {
            val voices = mapOf(
                -200L to "zh-CN-XiaoxiaoNeural",
                -201L to "zh-CN-YunjianNeural",
                -202L to "zh-CN-XiaoyiNeural",
                -203L to "zh-CN-YunxiNeural",
                -204L to "zh-CN-YunyangNeural",
                -205L to "zh-CN-XiaohanNeural"
            )
    ...
    val MIGRATION_75_76 = object : Migration(75, 76) {
        override fun migrate(db: SupportSQLiteDatabase) {
            val loginUi = """[{"name":"subscriptionKey","type":"password"},{"name":"region","type":"text"}]"""
            db.execSQL(
                """UPDATE httpTTS SET url = '', streamMode = 0,
                    apiFormat = 'audio-24khz-48kbitrate-mono-mp3', loginUi = ?
